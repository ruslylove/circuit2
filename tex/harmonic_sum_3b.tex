...
\begin{tikzpicture}
    \begin{axis}[
        ...
        % Third harmonic v3b = 1.5cos(3w0t)
        \addplot[red, dashed] {1.5*cos(deg(3*x))};
        \addlegendentry{$v_{3b} = 1.5\cos(3\omega_0 t)$}
        
        % Summation
        \addplot[black, ultra thick] {2*cos(deg(x)) + 1.5*cos(deg(3*x))};
        \addlegendentry{$v(t)$}
    \end{axis}
\end{tikzpicture}
\end{document}
