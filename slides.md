---
theme: seriph
background: https://cover.sli.dev
title: Circuit Analysis Technique
info: |
  ## Circuit Analysis Technique
  Introduction and Chapter 16
drawings:
  persist: false
transition: slide-left
mdc: true
---

# 010153105 Circuit Analysis Technique

## Part III (Chapter 16 & 18)

Frequency Response and Fourier Analysis

<div class="abs-br m-6 flex gap-2">
  <button @click="$slidev.nav.openInEditor()" title="Open in Editor" class="slidev-icon-btn">
    <carbon:edit />
  </button>
  <a href="https://github.com/slidevjs/slidev" target="_blank" class="slidev-icon-btn">
    <carbon:logo-github />
  </a>
</div>

---
transition: fade-out
---

# General Information

- **Course Name**: Circuit Analysis Technique
- **Course Number**: 010153105
- **Credits**: 3
- **Lecturers**:
  - Part 1: Assist. Prof. Dr. Pisit Vanichchanunt
  - Part 2: Dr. Bancha Janthong
  - Part 3: Assist. Prof. Dr. Ruslee Sutthaweekul

---

# Objectives

- Use techniques for solving linear circuits:
  - <span v-mark.underline.red=1>Fourier series and transform</span>
  - Laplace transform
  - Two-port networks
- Model and analyze linear circuits using LTI system concepts
- <span v-mark.underline.red=1>Analyze linear circuits </span> in time and <span v-mark.underline.red=1>frequency domains</span>
- Model and analyze magnetically coupled circuits

---

# Objectives (Cont.)

- Explain terms like:
  - <span v-mark.underline.red=1>Complex frequency</span>, Convolution, <span v-mark.underline.red=1>Frequency response</span>
  - Transfer function, Poles and Zeros, Impulse response
  - <span v-mark.underline.red=1>Parseval theorem, Energy spectral density </span>
  - <span v-mark.underline.red=1>Resonance, Quality factor, Bode plot</span>
  - <span v-mark.underline.red=1>Filters (low-pass, high-pass, etc.)</span>
  - Transformers

---

# References

**Textbook**
- William H. Hayt, Jr., Jack E. Kemmerly, and Steven M. Durbin, *Engineering Circuit Analysis*, 8 & 9th Editions, McGraw-Hill.

**Further Reading**
- Richard C. Dorf and James A. Svoboda, *Introduction to Electric Circuits*, 8th Edition.
- J. David Irwin and R. Mark Nelms, *Basic Engineering Circuit Analysis*, 9th Edition.
- Charles K. Alexander and Matthew N. O. Sadiku, *Fundamentals of Electric Circuits*, 3rd Edition.

---

# Teaching Plan

| Week | Chapters | Topics |
| --- | --- | --- |
| 1 | Intro | Introduction |
| 2-5 | 14, 15 | Complex frequency, Laplace, s-domain, Convolution |
| **Test 1** | | **Chapters 14, 15** |
| 6-9 | 13, 17 | Magnetically coupled, Transformers, Two-port networks |
| **Test 2** | | **Chapters 13, 17** |
| 10-15 | 16, 18 | Frequency response, Bode, Fourier, System function |
| <span v-mark.circle.red=1>**Final Exam**</span> | | **Chapters 16, 18** |

---

# Grading Policy

<div class="grid grid-cols-2 gap-4">

<div>

## Marks
- **Test 1**: 30%
- **Test 2**: 30%
- **Final Exam**: 30%
- **Attendance & Activity**: 10%

</div>

<div>

## Criteria
- **A**: 80-100
- **B+**: 72-79
- **B**: 65-71
- **C+**: 57-64
- **C**: 50-56
- **D+**: 42-49
- **D**: 35-41
- **F**: 0-34

</div>

</div>

---
src: ./chapter_16_p1.md
---
---
src: ./chapter_16_p2.md
---
---
src: ./chapter_16_p3.md
---
---
src: ./chapter_18_p1.md
---
---
src: ./chapter_18_p2.md
---

---

# Future Chapters

> Placeholder for future slides.

